...
(SELECT DISTINCT NULL AS STUDENT_ID,
      ...
      (SELECT SUM(RPRATRM_ACCEPT_AMT)
       ...
       (SELECT SUM (SFRSTCR_CREDIT_HR)
         ...
         WHERE     SFRSTCR_PIDM = RORSTAT_PIDM
               ...
               AND SFRSTCR_RSTS_CODE IN ('RR','RE','RW','R2')) SMR_REG_HRS,
       ...
       CASE WHEN FE.RORENRL_TERM_CODE = :SMR_TERM THEN FE.RORENRL_FINAID_BILL_HR END AS SMR_FEDAID_BILLED_HRS,
               
       (SELECT SUM (SFRSTCR_CREDIT_HR)
          FROM SFRSTCR
         WHERE     SFRSTCR_PIDM = RORSTAT_PIDM
               AND SFRSTCR_TERM_CODE = :FAL_TERM
               AND SFRSTCR_RSTS_CODE IN ('RR','RE','RW','R2')) FAL_REG_HRS,
       ...
       CASE WHEN FE.RORENRL_TERM_CODE = :FAL_TERM THEN FE.RORENRL_FINAID_BILL_HR END AS FAL_FEDAID_BILLED_HRS,
               
       (SELECT SUM (SFRSTCR_CREDIT_HR)
          FROM SFRSTCR
         WHERE     SFRSTCR_PIDM = RORSTAT_PIDM
               AND SFRSTCR_TERM_CODE = :SPR_TERM
               AND SFRSTCR_RSTS_CODE IN ('RR','RE', 'RW', 'R2')) SPR_REG_HRS,
       ...
       CASE WHEN FE.RORENRL_TERM_CODE = :SPR_TERM THEN FE.RORENRL_FINAID_BILL_HR END AS SPR_FEDAID_BILLED_HRS,
  ...
  FROM RORSTAT
       ...
       LEFT JOIN RORENRL FE
                   ON     RORSTAT_PIDM = FE.RORENRL_PIDM
                      AND FE.RORENRL_TERM_CODE IN ( :SMR_TERM, :FAL_TERM, :SPR_TERM)
                      AND FE.RORENRL_ENRR_CODE = 'FEDAID'
                      
       LEFT JOIN RORENRL SE
                   ON     RORSTAT_PIDM = SE.RORENRL_PIDM
                      AND SE.RORENRL_TERM_CODE IN ( :SMR_TERM, :FAL_TERM, :SPR_TERM)
